        ...
        bundle.bundle(opts.bundle_opts, function (err, src) {
            ...
            if (opts.write_file) {
                fs.writeFile(opts.write_file, cache, function (err) {
                    ...
                    if (opts.verbose) {
                        console.log('browserify -- writing file', opts.write_file);
                    }
                });
            }
        ...
        watchr.watch({
            ...
            listeners: {
                ...
                watching: function (err, instance, watching) {
                    if (err) {
                        console.log('browserify -- failed to watch', instance.path);
                    } else {
                        console.log('browserify -- watching', instance.path);
                    }
                },
            ...
            next: function (err, watchers) {
                if (err) {
                    return console.log('browserify -- failed', err);
                }
            }
        });
    }
